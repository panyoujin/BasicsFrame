...
namespace BF.Common.DataAccess.Helper
{
    public class MySqlHelper
    {
        ...
        public static void SetValue<T>(string columnName, T model, object objValue, PropertyInfo item)
        {
            if (objValue == null)
            {
                return;
            }
            //foreach (PropertyInfo item in itemArray)
            //{
            string propName = item.Name;
            if (string.Compare(propName, columnName, true) != 0)
            {
                return;
            }
            try
