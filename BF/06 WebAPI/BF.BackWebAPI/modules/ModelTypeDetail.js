define(['domready!', 'zepto', 'common', 'angular'], function (doc, $, c, angular) {
    $(function () {
        ...
        if (ID > 0) {//如果是编辑信息
            var url = window.apibase + "/Back_ModelType/QueryModelTypeByID";
        ...
        $("#save_ModelType").click(function () {
            if (formvalidate()) {
                var url = window.apibase + "/Back_ModelType/InsertModelType";
                c.post($('#tab').serialize(), url, function (data) {
                    if (data != null && data.code == "200") {
                        window.location.href = "Model_TypeManage.html";
                    }
                })
            }
